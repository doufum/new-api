...
export function AppHeader({
  ...
  const renderTopNavInActions =
    showTopNav && !leftContent && !rightContent && links.length > 0

  const leftSection =
    leftContent ||
    (showTopNav && !renderTopNavInActions ? <TopNav links={links} /> : null)
      ...
      <Header>
        ...
        {rightContent ?? (
          <div className='ms-auto flex items-center gap-3 sm:gap-4'>
            {renderTopNavInActions && <TopNav links={links} />}
            {showSearch && <Search />}
            {showNotifications && (
              <NotificationButton
